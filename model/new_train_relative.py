...
from model.CNN_hidden_mask import NeuralNetwork
...
for e in range(0, n_epochs):
    ...
    for b in range(0, n_sims):
        ...
        # state = (normalized min time (min time bound - current time / maxT),
        #         normalized (max time bound - current time / maxT),
        #         distances to other nodes from current location,
        #         prizes (zero for nodes where the agent has already been),
        #         visited modes (boolean mask))
    ...
    if e in [10, 20, 30, 40, 50, 60, 70, 80, 90, 95, 100, 120, 120, 170, 200]:
    ...
    while n_success_sims < n_sims_val + 1:
        ...
        while step_ < n_nodes and not if_break:
            ...
            actions_values = torch.where(actions_values * (1 - visited_nodes_ones) == 0, 2*model.threshold, actions_values.double()).squeeze(1)
            probs = nn.Softmax(dim=1)(actions_values)
            ...
            # all this "try", "except" staff is not necessary
            # (it was for safety at the beginning, but we never get to "except" options)
            try:
                tour_time, time_t, rwd_t, pen_t, feas, violation_t = env_val.step(torch.add(next_nodes, 1).int())
                tour_time = torch.from_numpy(tour_time).to(device)
            except:
                tour_time = torch.zeros(1, 1, dtype=torch.int64, device=device)
                if_break = True
            next_states = torch.cat(((min_window - tour_time.unsqueeze(1) * multiplier) / max_time,
                                     (max_window - tour_time.unsqueeze(1) * multiplier) / max_time,
